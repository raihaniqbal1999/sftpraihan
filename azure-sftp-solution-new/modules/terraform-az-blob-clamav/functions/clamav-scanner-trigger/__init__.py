...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    scanContGroup = os.environ["SCANNER_CONTAINER_GROUP"]

    utc_timestamp = (
        datetime.datetime.utcnow().replace(tzinfo=datetime.timezone.utc).isoformat()
    )

    logger.info("ClamAV scanner triggered at {0}".format(utc_timestamp))
    ...
    update_cg = aci_client.container_groups.get(resGroup, scanContGroup)
    ...
    if update_cg.containers[0].instance_view.current_state.state == "Running":
        ...
        start_containers(aci_client, resGroup, scanContGroup)
